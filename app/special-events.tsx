                ...
                <article key={event.slug} className="relative isolate flex flex-col gap-8 sm:flex-row">
                  <div className="relative aspect-[16/9] sm:w-64 md:w-72 sm:shrink-0">
                    ...
                    <p className="absolute bottom-2 left-2 z-10 rounded-full text-xs bg-opacity-85 bg-gray-100 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100">
                        {Date.now() < new Date(event.date).getTime() ? "Upcoming" : "Ended"}
                    </p>
                  </div>
                  <div>
                    ...
                    <div className="flex flex-col items-start gap-x-4 text-xs">
                      <time dateTime={event.date} className="text-gray-500 mt-2">
                        {new Intl.DateTimeFormat("en-GB", {timeZone: "UTC", weekday: "short", year: "numeric", month: "short", day:"2-digit", hour:"2-digit", minute:"2-digit", hour12: true}).format(new Date(event.date))}
                        -
                        {new Intl.DateTimeFormat("en-GB", {timeZone: "UTC", hour:"2-digit", minute:"2-digit", hour12: true}).format(new Date(new Date(event.date).getTime() + event.duration*60000))}
                      </time> 
                      {event.date2 && <time dateTime={event.date} className="text-gray-500 mt-2">
                        {new Intl.DateTimeFormat("en-GB", {timeZone: "UTC", weekday: "short", year: "numeric", month: "short", day:"2-digit", hour:"2-digit", minute:"2-digit", hour12: true}).format(new Date(event.date2))}
                        -
                        {new Intl.DateTimeFormat("en-GB", {timeZone: "UTC", hour:"2-digit", minute:"2-digit", hour12: true}).format(new Date(new Date(event.date2).getTime() + event.duration2*60000))}
                      </time>} 
                      <Link
